...
class Profile extends UActiveRecord
{
	...
	{
		return Yii::app()->getModule('kuser')->tableProfiles;
	}
	...
	public function rules()
	{
		...
		foreach ($model as $field) {
			...
			if ($field->field_type=='VARCHAR'||$field->field_type=='TEXT') {
				...
				if ($field->error_message) $field_rule['message'] = KuserModule::t($field->error_message);
	...
	public function relations()
	{
		...
		if (isset(Yii::app()->getModule('kuser')->profileRelations)) $relations = array_merge($relations,Yii::app()->getModule('kuser')->profileRelations);
		return $relations;
	}
	...
	{
		$labels = array(
			'user_id' => KuserModule::t('User ID'),
		);
		$model=$this->getFields();
		
		foreach ($model as $field)
			$labels[$field->varname] = ((Yii::app()->getModule('kuser')->fieldsMessage)?KuserModule::t($field->title,array(),Yii::app()->getModule('kuser')->fieldsMessage):KuserModule::t($field->title));
