...
class ProfileFieldController extends Controller
{
	...
	var fieldType = {
			...
			'BOOL':{
				...
				'val':{
					'field_size':0,
					'default':0,
					'range':'1==".KuserModule::t('Yes').";0==".KuserModule::t('No')."',
					'widgetparams':''
				}
			},
			'BLOB':{
	...
	function showWidgetList(type) {
		...
		$('div.widget select').append('<option value=\"\">".KuserModule::t('No')."</option>');
		if (wgByType[type]) {
	...
	// show all function 
	$('div.form p.note').append('<br/><a href=\"#\" id=\"showAll\">".KuserModule::t('Show all')."</a>');
	...
	public static function getWidgets($fieldType='') {
		$basePath=Yii::getPathOfAlias('application.modules.kuser.components');
		$widgets = array();
		$list = array(''=>KuserModule::t('No'));
		if (self::$_widgets) {
